...
use crate::{
    action::{Action, ZAction, ZA_MINTAUTH},
    address::Address,
...
impl Bundle
{
    ...
    pub fn proof<R: RngCore>(&self, mut rng: R) -> (Proof, Vec<Circuit>)
    {
        ...
        self.0.iter().for_each(|a| {
            ...
            if a.za_type() != ZA_MINTAUTH
            {
                ...
                let fvk = a.fvk_a().get_or_insert(&dummy_fvk).clone();
                ...
                circuits.push(Circuit {
                    ...
                    rho_b: Value::known(note_a.nullifier(&fvk)),
        ...
        (Proof::create(&pk, &circuits, &instances, rng).unwrap(), circuits)
    }

    /// Prepares a bundle for private transaction by calculating proof, the list of zactions and the encrypted note data
    pub fn prepare<R: RngCore>(&self, mut rng: R) -> ((Proof, Vec<Circuit>), Vec<ZAction>, Vec<TransmittedNoteCiphertext>)
    {
...
mod tests
{
    ...
    use crate::{
        ...
        value::{NoteValue}, action::ZA_TRANSFERFT, tree::MerklePath,
        circuit::{Circuit, K}
    };
    ...
    fn ft_transfer()
    {
        ...
        let ((proof, circuits), zactions, encrypted_notes) = bundle.prepare(rng);
        ...
        zactions.iter().for_each(|za| instances.push(za.instance()));
        // mock prover
